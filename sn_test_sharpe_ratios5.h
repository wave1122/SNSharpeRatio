...
class SnTestSharpeR{
	...
	/* Calculate the lengths of the 99%-, 95%-, and 90%-confidence intervals of the difference between two Sharpe ratios
	based on the self-normalized test statistic */
	static matrix<double> ci_len_sn_sharpe_stat(const matrix<double> &X, /* excess returns on first asset */
												const matrix<double> &Y /* excess returns on second asset */);
...
/* Calculate the lengths of the 99%-, 95%-, and 90%-confidence intervals of the difference between two Sharpe ratios
	based on the self-normalized test statistic */
matrix<double> SnTestSharpeR::ci_len_sn_sharpe_stat(const matrix<double> &X, /* excess returns on first asset */
													const matrix<double> &Y /* excess returns on second asset */) {
	ASSERT_(X.nr() == Y.nr());
	int T = X.nr();
...
//		cout << "var = " << var << endl;
	...
	matrix<double,1,3> cvs, ci_lens;
	cvs = 99.76,
		  45.40,
		  28.31;
	ci_lens = 2*sqrt(var/T)*dlib::sqrt(cvs);
//	cout << "ci_lens = " << ci_lens << endl;

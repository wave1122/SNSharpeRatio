...
//	cout << mu1 << endl;
	...
	dir_name = "./Results/non_nested/argarch/fovl=" + std::to_string(fovl) + "/";
	...
	string stats_filename = dir_name + "stats_err_dist=" + err_dist + "_kernel=" + kernel + ".txt";
	...
	string size_filename = dir_name + "size_err_dist=" + err_dist + "_kernel=" + kernel + ".txt";
	...
	string pwr_filename = dir_name + "power_err_dist=" + err_dist + "_kernel=" + kernel + ".txt";
	pwr_out.open(pwr_filename, ios::out);

	pwr_out << "kernel bandwidths = " << endl;
	pwr_out << bws << endl;

	pwr_out << "Block sizes = " << endl;
	pwr_out << Ls << endl;
	...
	matrix<double> REJF1a(2,1), REJF1b(Ls.nr()+2,1), REJF1(Ls.nr()+3,1);
	matrix<double> REJF5a(2,1), REJF5b(Ls.nr()+2,1), REJF5(Ls.nr()+3,1);
	matrix<double> REJF10a(2,1), REJF10b(Ls.nr()+2,1), REJF10(Ls.nr()+3,1);
	while (T <= 750) {
		cout << "T = " << T << endl;
		stats_out << "T = " << T << endl;
		cout << "mu = \n" << mu1 << endl;
		...
		auto [asymp_REJF1, asymp_REJF5, asymp_REJF10, boot_REJF1, boot_REJF5, boot_REJF10, sn_REJF1, sn_REJF5, sn_REJF10]
										 = Power::power_fovl<QS_kernel,
															Dgp::gen_dvech_margarch<gsl_ran_gaussian>>(num_samples, /* number of random samples */
																									T, /* sample size */
		...
		stats_out << "mu = \n" << mu2 << endl;
		std::tie(asymp_REJF1, asymp_REJF5, asymp_REJF10, boot_REJF1, boot_REJF5, boot_REJF10, sn_REJF1, sn_REJF5, sn_REJF10)
										 = Power::power_fovl<QS_kernel,
															Dgp::gen_dvech_margarch<gsl_ran_gaussian>>(num_samples, /* number of random samples */
																									T, /* sample size */
																									bws, /* kernel bandwidths */
		...
		pwr_out << "T = " << T << endl;
		pwr_out << "mu = \n" << mu2 << endl;
		pwr_out << "1%-level rejection rates:" << endl;
		sn_REJF_vec = sn_REJF1;
		REJF1a = join_rows(bws, asymp_REJF1);
		REJF1b = join_rows(REJF1a, boot_REJF1);
		REJF1 = join_rows(REJF1b, sn_REJF_vec);
		...
		pwr_out << "=======================================================================================================================" << endl;
		pwr_out << "5%-level rejection rates:" << endl;
		sn_REJF_vec = sn_REJF5;
		REJF5a = join_rows(bws, asymp_REJF5);
		REJF5b = join_rows(REJF5a, boot_REJF5);
		REJF5 = join_rows(REJF5b, sn_REJF_vec);
		...
		pwr_out << "=======================================================================================================================" << endl;
		pwr_out << "10%-level rejection rates:" << endl;
		sn_REJF_vec = sn_REJF10;
		REJF10a = join_rows(bws, asymp_REJF10);
		REJF10b = join_rows(REJF10a, boot_REJF10);
		REJF10 = join_rows(REJF10b, sn_REJF_vec);

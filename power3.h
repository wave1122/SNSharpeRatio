...
#include <sn_test_sharpe_ratios5.h>
...
class Power{
	public:
		...
		/* Calculate empirical rejection frequencies for Student's t-test, the bootstrap Student's t-test,
		and the self-normalized t-test for the null hypothesis of equality between two Sharpe ratios */
		...
		/* Calculate the lengths of 99%- 95%-, and 90%-confidence intervals of the difference between two Sharpe ratios based on Student's t-test,
		the bootstrap Student's t-test, and the self-normalized t-test */
		template</* a HAC kernel */
				double kernel(double),
				/* a data generating process */
				std::pair<matrix<double>, matrix<double>> gen_data(const int, /* number of random samples */
																	...
																	const string,          /* distribution of the errors */
																	unsigned long          /* a seed to generate random numbers */)>
		static std::tuple<matrix<double>, matrix<double>, matrix<double>,  /* mean of CI lengths based on Student's t-test */
							matrix<double>, matrix<double>, matrix<double>, /* standard deviation of CI lengths based on Student's t-test */
							matrix<double>, matrix<double>, matrix<double>, /* mean of CI lengths based on the bootstrapped Student's t-test */
							matrix<double>, matrix<double>, matrix<double>, /* standard deviation of CI lengths based on the bootstrapped Student's t-test */
							double, double, double, 						/* mean of CI lengths based on the self-normalized test */
							double, double, double 						/* standard deviation of CI lengths based on the self-normalized test */>
																		ci_lens(const int num_samples, /* number of random samples */
																				const int T, /* sample size */
																				const matrix<double> &bws, /* kernel bandwidths */
																				const matrix<int> &Ls, /* block sizes */
																				const int num_boots, /* number of bootstrap repetitions */
...
template</* a HAC kernel */
		...
		std::pair<matrix<double>, matrix<double>> gen_data(const int, /* number of random samples */
															...
															const string,          /* distribution of the errors */
															unsigned long          /* a seed to generate random numbers */)>
std::tuple<matrix<double>, matrix<double>, matrix<double>,  /* mean of CI lengths based on Student's t-test */
			matrix<double>, matrix<double>, matrix<double>, /* standard deviation of CI lengths based on Student's t-test */
			matrix<double>, matrix<double>, matrix<double>, /* mean of CI lengths based on the bootstrapped Student's t-test */
			matrix<double>, matrix<double>, matrix<double>, /* standard deviation of CI lengths based on the bootstrapped Student's t-test */
			double, double, double,         				/* mean of CI lengths based on the self-normalized test */
			double, double, double 						/* standard deviation of CI lengths based on the self-normalized test */>
													     Power::ci_lens(const int num_samples, /* number of random samples */
																		const int T, /* sample size */
																		const matrix<double> &bws, /* kernel bandwidths */
																		const matrix<int> &Ls, /* block sizes */
																		const int num_boots, /* number of bootstrap repetitions */
	...
	cout << (boost::format("Calculating CI lengths for T=%d, num_samples=%d, and num_boots=%d") %T %num_samples %num_boots).str() << endl;
	pwr_out << (boost::format("Calculating CI lengths for T=%d, num_samples=%d, and num_boots=%d") %T %num_samples %num_boots).str() << endl;
	pwr_out << "Distribution of the errors = " << err_dist << endl;
	pwr_out << "A = " << A << endl;
	pwr_out << "B = " << B << endl;
	pwr_out << "C = " << C << endl;
	...
	matrix<double> Xi(T,1), Yi(T,1);
	matrix<double> ci_lens_t_stat(num_samples, 3), ci_lens_boot_t_stat(num_samples, 3), ci_lens_sn_stat(num_samples, 3);

	int num_bws = bws.nr(), num_blocks = Ls.nr();

	matrix<double> ci_lens_t_stat_mean_99(num_bws,1), ci_lens_t_stat_mean_95(num_bws,1), ci_lens_t_stat_mean_90(num_bws,1);
	matrix<double> ci_lens_t_stat_std_99(num_bws,1), ci_lens_t_stat_std_95(num_bws,1), ci_lens_t_stat_std_90(num_bws,1);

	matrix<double> ci_lens_boot_t_stat_mean_99(num_bws,num_blocks), ci_lens_boot_t_stat_mean_95(num_bws,num_blocks), ci_lens_boot_t_stat_mean_90(num_bws,num_blocks);
	matrix<double> ci_lens_boot_t_stat_std_99(num_bws,num_blocks), ci_lens_boot_t_stat_std_95(num_bws,num_blocks), ci_lens_boot_t_stat_std_90(num_bws,num_blocks);

	double ci_lens_sn_stat_mean_99 = 0., ci_lens_sn_stat_mean_95 = 0., ci_lens_sn_stat_mean_90 = 0.;
	double ci_lens_sn_stat_std_99 = 0., ci_lens_sn_stat_std_95 = 0., ci_lens_sn_stat_std_90 = 0.;


	pwr_out << std::fixed << std::setprecision(5);

	/* Calculate the Length of Confidence Intervals based on Student's t-test */
	pwr_out << "===================================================================================" << endl;
	pwr_out << "99%-, 95%-, and 90%-CI lengths based on Student's t-test" << endl;
	for (int k = 0; k < num_bws; ++k) {
		ci_lens_t_stat = 0.; //reset the matrix to zeros
		for (i = 0; i < num_samples; ++i) {
			try {
				Xi = colm(X,i);
				Yi = colm(Y,i);

				//calculate the lengths of confidence intervals of the difference between two Sharpe ratios based on Student's t-test
				set_rowm(ci_lens_t_stat,i) = TestSharpeR::ci_lens_t_stat_diff_sharpes<kernel>(Xi, /* excess returns on first asset */
																							  Yi, /* excess returns on second asset */
																							  bws(k) /* kernel bandwidth */);

				pwr_out << "bandwidth (" << k << ") = " << bws(k) << endl;
				pwr_out << "CI lengths: " << rowm(ci_lens_t_stat,i) << endl;
			}
			catch(...) { //catch all exceptions
					cerr << "Power::ci_lens: An exception occurs while calculating the CI based on Student's t-test!" << endl;
					++skip;
			}
		}

		ci_lens_t_stat_mean_99(k) = dlib::sum(colm(ci_lens_t_stat,0))/(num_samples-skip);
		ci_lens_t_stat_std_99(k) = sqrt(dlib::sum(dlib::pow(colm(ci_lens_t_stat,0)- ci_lens_t_stat_mean_99(k),2.))/(num_samples-skip));
		ci_lens_t_stat_mean_95(k) = dlib::sum(colm(ci_lens_t_stat,1))/(num_samples-skip);
		ci_lens_t_stat_std_95(k) = sqrt(dlib::sum(dlib::pow(colm(ci_lens_t_stat,1)- ci_lens_t_stat_mean_95(k),2.))/(num_samples-skip));
		ci_lens_t_stat_mean_90(k) = dlib::sum(colm(ci_lens_t_stat,2))/(num_samples-skip);
		ci_lens_t_stat_std_90(k) = sqrt(dlib::sum(dlib::pow(colm(ci_lens_t_stat,2)- ci_lens_t_stat_mean_90(k),2.))/(num_samples-skip));

		pwr_out << "The mean and standard deviation of lengths of 99%, 95%, and 90%-confidence intervals based on Student's t-test:" << endl;
		pwr_out << "k = " << bws(k) << " , "  << ci_lens_t_stat_mean_99(k) << " , " << ci_lens_t_stat_mean_95(k) << " , " << ci_lens_t_stat_mean_90(k) << endl;
		pwr_out << "k = " << bws(k) << " , "  << ci_lens_t_stat_std_99(k) << " , " << ci_lens_t_stat_std_95(k) << " , " << ci_lens_t_stat_std_90(k) << endl;
		cout << "k = " << bws(k) << " , "  << ci_lens_t_stat_mean_99(k) << " , " << ci_lens_t_stat_mean_95(k) << " , " << ci_lens_t_stat_mean_90(k) << endl;
		cout << "k = " << bws(k) << " , "  << ci_lens_t_stat_std_99(k) << " , " << ci_lens_t_stat_std_95(k) << " , " << ci_lens_t_stat_std_90(k) << endl;
	}


	/* Calculate the Length of Confidence Intervals based on the Bootstrapped Student's t-test */
	pwr_out << "====================================================================================================" << endl;
	pwr_out << "99%-, 95%-, and 90%-CI lengths based on the bootstrapped Student's t test" << endl;
	for (int k = 0; k < num_bws; ++k) {
		for (int h = 0; h < num_blocks; ++h) {
			ci_lens_boot_t_stat = 0.; //reset the matrix to zero
//			pwr_out << (boost::format("bw = %f and block size = %d") %bw %L).str() << endl;
			skip = 0;
			#pragma omp parallel for default(shared) reduction(+:skip) schedule(static,CHUNK) private(i) firstprivate(Xi,Yi)
			for (i = 0; i < num_samples; ++i) {
				try {
					Xi = colm(X,i);
					Yi = colm(Y,i);

					/* Calculate the lengths of confidence intervals of the difference between two Sharpe ratios */
					set_rowm(ci_lens_boot_t_stat,i) = TestSharpeR::ci_lens_t_boot_diff_sharpes<kernel>(Xi, /* excess returns on first asset */
																										Yi, /* excess returns on second asset */
																										bws(k), /* kernel bandwidth */
																										Ls(h), /* block size */
																										num_boots, /* number of bootstrap repetitions */
					...
					#pragma omp critical
					{
						pwr_out << "bandwidth (" << k << ") = " << bws(k) << " , " << "block size (" << h << ") = " << Ls(h) <<  endl;
						pwr_out << "CI lengths: " << rowm(ci_lens_boot_t_stat,i) << endl;
//						cout << bws(k) << " , " << Ls(h) << endl;
					}
				}
				catch(...) { //catch all exceptions
					cerr << "Power::ci_lens: An exception occurs while calculating the CI based on the bootstrap t-test!" << endl;
					++skip;
				}
			}

			ci_lens_boot_t_stat_mean_99(k,h) = dlib::sum(colm(ci_lens_boot_t_stat,0))/(num_samples-skip);
			ci_lens_boot_t_stat_std_99(k,h) = sqrt(dlib::sum(dlib::pow(colm(ci_lens_boot_t_stat,0)- ci_lens_boot_t_stat_mean_99(k,h),2.))/(num_samples-skip));
			ci_lens_boot_t_stat_mean_95(k,h) = dlib::sum(colm(ci_lens_boot_t_stat,1))/(num_samples-skip);
			ci_lens_boot_t_stat_std_95(k,h) = sqrt(dlib::sum(dlib::pow(colm(ci_lens_boot_t_stat,1)- ci_lens_boot_t_stat_mean_95(k,h),2.))/(num_samples-skip));
			ci_lens_boot_t_stat_mean_90(k,h) = dlib::sum(colm(ci_lens_boot_t_stat,2))/(num_samples-skip);
			ci_lens_boot_t_stat_std_90(k,h) = sqrt(dlib::sum(dlib::pow(colm(ci_lens_boot_t_stat,2)- ci_lens_boot_t_stat_mean_90(k,h),2.))/(num_samples-skip));

			pwr_out << "The mean and standard deviation of the lengths of 99%, 95%, and 90%-confidence intervals based on the bootstrapped Student's t-test:" << endl;
			pwr_out << "(k, h) = (" << bws(k) << " , " << Ls(h) << "):  " << ci_lens_boot_t_stat_mean_99(k,h) << " , " << ci_lens_boot_t_stat_mean_95(k,h) << " , "
																														<< ci_lens_boot_t_stat_mean_90(k,h) << endl;
			pwr_out << "(k, h) = (" << bws(k) << " , " << Ls(h) << "): " << ci_lens_boot_t_stat_std_99(k,h) << " , " << ci_lens_boot_t_stat_std_95(k,h) << " , "
																															<< ci_lens_boot_t_stat_std_90(k,h) << endl;
			cout << "(k, h) = (" << bws(k) << " , " << Ls(h) << "):  " << ci_lens_boot_t_stat_mean_99(k,h) << " , " << ci_lens_boot_t_stat_mean_95(k,h) << " , "
																														<< ci_lens_boot_t_stat_mean_90(k,h) << endl;
			cout << "(k, h) = (" << bws(k) << " , " << Ls(h) << "): " << ci_lens_boot_t_stat_std_99(k,h) << " , " << ci_lens_boot_t_stat_std_95(k,h) << " , "
																															<< ci_lens_boot_t_stat_std_90(k,h) << endl;
		}
	}

	/* Calculate the Length of Confidence Intervals based on the self-normalized test statistics */
	pwr_out << "====================================================================================" << endl;
	pwr_out << "The 99%-, 95%-, and 90%-CI lengths based on the self-normalized test" << endl;
	skip = 0;
	ci_lens_sn_stat = 0.; //reset the matrix to zero
	for (i = 0; i < num_samples; ++i) {
		try {
			Xi = colm(X,i);
			Yi = colm(Y,i);
			...
			/* Calculate the self-normalized t-statistic of the difference between two Sharpe ratios */
			set_rowm(ci_lens_sn_stat,i) = SnTestSharpeR::ci_len_sn_sharpe_stat(Xi,Yi);

			pwr_out << "CI lengths: " << rowm(ci_lens_sn_stat,i) << endl;
//			cout << sn_stat << endl;

		}
		catch(...) { //catch all exceptions
			cerr << "Power::ci_lens: An exception occurs while calculating the CI based on the self-normalized test!" << endl;
			++skip;
		}
	}

	ci_lens_sn_stat_mean_99 = dlib::sum(colm(ci_lens_sn_stat,0))/(num_samples-skip);
	ci_lens_sn_stat_std_99 = sqrt(dlib::sum(dlib::pow(colm(ci_lens_sn_stat,0)- ci_lens_sn_stat_mean_99,2.))/(num_samples-skip));
	ci_lens_sn_stat_mean_95 = dlib::sum(colm(ci_lens_sn_stat,1))/(num_samples-skip);
	ci_lens_sn_stat_std_95 = sqrt(dlib::sum(dlib::pow(colm(ci_lens_sn_stat,1)- ci_lens_sn_stat_mean_95,2.))/(num_samples-skip));
	ci_lens_sn_stat_mean_90 = dlib::sum(colm(ci_lens_sn_stat,2))/(num_samples-skip);
	ci_lens_sn_stat_std_90 = sqrt(dlib::sum(dlib::pow(colm(ci_lens_sn_stat,2)- ci_lens_sn_stat_mean_90,2.))/(num_samples-skip));

	pwr_out << "The mean and standard deviation of lengths of 99%, 95%, and 90%-confidence intervals based on the self-normalized test:" << endl;
	pwr_out << ci_lens_sn_stat_mean_99 << " , " << ci_lens_sn_stat_mean_95 << " , " << ci_lens_sn_stat_mean_90 << endl;
	pwr_out << ci_lens_sn_stat_std_99 << " , " << ci_lens_sn_stat_std_95 << " , " << ci_lens_sn_stat_std_90 << endl;
	cout << ci_lens_sn_stat_mean_99 << " , " << ci_lens_sn_stat_mean_95 << " , " << ci_lens_sn_stat_mean_90 << endl;
	cout << ci_lens_sn_stat_std_99 << " , " << ci_lens_sn_stat_std_95 << " , " << ci_lens_sn_stat_std_90 << endl;
	pwr_out << "=======================================================================================================================" << endl;

	return {ci_lens_t_stat_mean_99, ci_lens_t_stat_std_99, ci_lens_t_stat_mean_95, ci_lens_t_stat_std_95, ci_lens_t_stat_mean_90, ci_lens_t_stat_std_90,
